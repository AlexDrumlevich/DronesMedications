...
@Query("select d.number as number, count(log.drone) as amount from EventLog log"
	...
	@Query("SELECT medication FROM EventLog WHERE drone.number = :droneNumber ORDER BY timestamp DESC LIMIT 1")
	Medication findTopByDroneNumber(String droneNumber);
